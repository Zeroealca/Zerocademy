...
- `TeacherAssignment` owns both the `AcademicPlan → AcademicUnit → LessonPlan` planning context and `ClassSession` execution occurrences. Planning content expresses pedagogical intent; a ClassSession records an actual occurrence and may optionally reference a compatible LessonPlan. Assignment deletion is restricted while historical sessions exist.
