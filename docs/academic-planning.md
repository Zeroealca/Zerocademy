...
**Phase 2C.1, 2C.2, 2C.3, and 2C.4 are technically complete.** Phase 2C.2 exposes the assignment-scoped backend API at `/v1/teacher-assignments/:teacherAssignmentId/class-sessions`, and Phase 2C.3 hardens its service and OpenAPI contracts. Phase 2C.4 provides frontend ClassSession contracts, the assignment-scoped API client, and TanStack Query list/detail queries plus create/update cache synchronization. **Phase 2C.5 is in progress:** 2C.5a adds the read-only execution workspace. The supporting 2C.5b.1a slice adds an aggregate Planning read for compatible LessonPlans; 2C.5b's TEACHER-only create form and its selector remain pending, and edit remains deferred.
...
- **2C.4 — Frontend contracts and data access (technically complete):** `academic-execution` provides ClassSession response/create/update types, assignment-scoped list/detail/create/update API-client methods, and TanStack Query list/detail queries plus create/update mutations. Query keys retain `TeacherAssignment → ClassSession` nested identity: list invalidation and detail synchronization remain isolated to the affected assignment. UI remains deferred to Phase 2C.5.
- **2C.5 — Execution workspace UI (in progress):** 2C.5a provides the assignment-scoped read workspace at `/academic-plans/:planId/execution`, reached from its parent AcademicPlan detail. It uses the ClassSession TanStack Query list hook and renders responsive status, scheduled-date, occurred-date, and optional LessonPlan-association information with loading, error, and empty states. TEACHER, ADMIN, and SUPER_ADMIN receive read presentation according to their existing scoped access; STUDENT and REPRESENTATIVE are excluded. The 2C.5b.1a support slice provides `GET /v1/academic-plans/:planId/lesson-plans`: a read-only aggregate projection of that plan's LessonPlans, ordered by AcademicUnit position then LessonPlan position. It reuses Planning read authorization and returns the normal LessonPlan response fields plus `academicUnitTitle`, enabling a later execution selector without frontend or service-level N+1 loading. Unit-scoped LessonPlan CRUD remains unchanged. The ClassSession create selector and edit workflow remain deferred.
...
### Aggregate AcademicPlan LessonPlan read (Phase 2C.5b.1a)

`GET /v1/academic-plans/:planId/lesson-plans` is a read-only aggregate projection for the ClassSession create workflow. It returns only LessonPlans whose AcademicUnit belongs to the requested AcademicPlan, ordered by `AcademicUnit.position ASC` and then `LessonPlan.position ASC`. Each item retains the existing LessonPlan response fields and adds `academicUnitTitle` for a human-readable selector label; it does not return the broader AcademicPlan graph.

The endpoint uses the same Planning read policy: the owning TEACHER, an institution-scoped ADMIN, and SUPER_ADMIN can read; other teachers, STUDENT, and REPRESENTATIVE are denied without exposing unrelated data. Published plans and historical/closed periods remain readable. It adds no mutation route, schema change, migration, or replacement for the existing Unit-scoped LessonPlan CRUD API.
